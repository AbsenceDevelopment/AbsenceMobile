...
class MnemonicUtils {
  ...
  static Future<String> generateMnemonic(Uint8List initialEntropy) async {
    ...
    //print("Length of WORD_LIST: ${WORD_LIST.length}\nLength of WORD_LIST[0]:${WORD_LIST[0].length}");
...
// Generate the master seed from the mnemonic and the passphrase [The passphrase is optional]
  ...
  static Future<List<String>> populateWordList() async {
    final contents = await rootBundle.loadString('assets/en-mnemonic-word-list.txt');
    //List<String> lines = .readAsLinesSync();
